...
import { Typography, message, Icon, Button } from "antd";
...
class App extends React.Component {
  ...
  render() {
    ...
    return (
      <Router>
        <AppLayout username={googleUsername} folderName={folderName}>
          {loggedIn ? (
            <Switch>
              <Route
                exact
                path="/"
                render={() => <Redirect to="/choose-folder" />}
              />
            ...
            <React.Fragment>
              {loggedIn === null && (
                <Text type="secondary">
                  <Icon type="loading" /> &nbsp; Checking Google login status
                </Text>
              )}
